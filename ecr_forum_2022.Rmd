...
```{r table for forum ideas, include = FALSE}
...
```{r table for forum ideas for Rob, eval=TRUE, include=TRUE}
...
knitr::kable (
  ...
  booktabs = T,
  linesep = "\\addlinespace",
  col.names = c("Response #", "ECR Forum Ideas"),
  caption =
    sprintf("Responses from %s completed surveys", paste(ns, collapse = " people from ")),
  align = 'cl', 
  longtable = T #break the table over multiple pages
...
```{r plot the training responses, include = FALSE}
...
```{r table for other training, include = FALSE}
...
```{r table for anything else, include = FALSE}
